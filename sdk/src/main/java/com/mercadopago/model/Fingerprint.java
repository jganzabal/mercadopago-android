  ...
  //  @JsonIgnore
    ...
    public ArrayList<VendorId> getVendorIds() {
        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.FROYO) {
            ...
            if (!TextUtils.isEmpty(id) && !"unknown".equals(id)) {
                vendorIds.add(new VendorId("serial", id));
            }
        }
    ...
    private class VendorId {
        ...
        public VendorId(String name, String value) {
            mName = name;
            mValue = value;
        }
    ...
    private class VendorSpecificAttributes {
        ...
        public boolean getFeatureFrontCamera() {
            return mContext.getPackageManager().hasSystemFeature("android.hardware.camera.front");
        }
        ...
        public boolean getFeatureGyroscope() {
            return mContext.getPackageManager().hasSystemFeature("android.hardware.sensor.gyroscope");
        }
    ...
    private class Location {
        ...
        public Location() {
            SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(mContext);
            try {
                mLocation =  new JSONObject(prefs.getString(SHARED_PREFS_FINGERPRINT_LOCATION, null));
        ...
        public Location(double latitude, double longitude) {
            ...
            try {
                mLocation.put(LOCATION_TIMESTAMP, System.currentTimeMillis() / 1000L);
                mLocation.put(LOCATION_LATITUDE, latitude);
                mLocation.put(LOCATION_LONGITUDE, longitude);
            } catch (JSONException ignored) {}
        }
/*
        @Override
        public String toString() {
            ObjectMapper mapper = new MLObjectMapper().getInstance();
            try {
...
*/
        ...
        public long getTimestamp() {
            if (mLocation != null) {
                try {
                    return mLocation.getLong(LOCATION_TIMESTAMP);
                } catch (JSONException ignored) {}
            }
        ...
        public double getLatitude() {
            if (mLocation != null) {
                try {
                    return mLocation.getDouble(LOCATION_LATITUDE);
                } catch (JSONException ignored) {}
            }
        ...
        public double getLongitude() {
            if (mLocation != null) {
                try {
                    return mLocation.getDouble(LOCATION_LONGITUDE);
                } catch (JSONException e) {}
            }
